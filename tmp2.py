...
def create_img_result(file_index, cols, rows, name_i):
    ...
    results_4class = np.zeros((rows, cols, 5), dtype=np.float32)
    ...
    # 道路
    img1_class4[img1_class4 > 0.01] = 1
    img1_class4[img1_class4 <= 0.01] = 0

    results_4class[:, :, 1] = img1_class1
    results_4class[:, :, 2] = img1_class2
    results_4class[:, :, 3] = img1_class3
    results_4class[:, :, 4] = img1_class4

    # 关键的一步,找到最大索引
    max_index = np.argmax(results_4class, axis=2)
